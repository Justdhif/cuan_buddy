...
class WidgetSettingsScreen extends ConsumerWidget {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text('Dashboard Balance', style: AppTypography.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
          ...
          Consumer(
            builder: (context, ref, _) {
              ...
              if (selectedGoalId != null) {
                try {
                  selectedGoalName = savingsState.goals.firstWhere((g) => g['id'] == selectedGoalId)['name'];
                } catch (e) {
                  // Not found
                }
              }
              ...
              return GestureDetector(
                onTap: () => _showSavingsSelectionSheet(context, ref, savingsState, selectedGoalId),
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(
                    color: isDark ? AppColors.surfaceDark : AppColors.surfaceLight,
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Savings Goal', style: AppTypography.textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
                            const SizedBox(height: 4),
                            Text(
                              selectedGoalName == null ? 'Tap to select a goal' : 'Active: $selectedGoalName',
                              style: AppTypography.textTheme.labelMedium?.copyWith(
                                color: selectedGoalName == null ? AppColors.primary : (isDark ? AppColors.textSecondaryDark : AppColors.textSecondaryLight),
                                fontWeight: selectedGoalName == null ? FontWeight.bold : FontWeight.normal,
                              ),
                            ),
                          ],
                        ),
                      ),
                      const Icon(Icons.chevron_right_rounded, color: AppColors.textHintLight),
                    ],
                  ),
                ),
              );
  ...
  void _showSavingsSelectionSheet(BuildContext context, WidgetRef ref, SavingsState savingsState, String? currentId) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
            ...
            child: Column(
              ...
              Expanded(
                child: savingsState.goals.isEmpty
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Text(goal['icon'] as String? ?? '🎯', style: const TextStyle(fontSize: 24)),
                      title: Text(goal['name'] as String? ?? 'Goal', style: AppTypography.textTheme.bodyLarge),
                      trailing: isSelected ? const Icon(Icons.check_circle_rounded, color: AppColors.primary) : null,
                      onTap: () async {
                        await ref.read(selectedSavingsWidgetIdProvider.notifier).setSelectedSavingsId(goal['id'] as String);
              ...
              if (currentId != null) ...[
                ...
                ListTile(
                  ...
                  title: Text('Disable Savings Widget', style: AppTypography.textTheme.bodyLarge?.copyWith(color: AppColors.danger)),
                  onTap: () async {
                    await ref.read(selectedSavingsWidgetIdProvider.notifier).setSelectedSavingsId(null);
                    if (context.mounted) Navigator.pop(context);
                  },
                ),
              ],
